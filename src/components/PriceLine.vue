    ...
    <div class="container">
        <div class="price-content">
            <div class="price-inside">
                ...
                <div v-for="(pricen, index) in namePrices" :key="index" class="price-item">
                  ...
                  <div class="price-name">
                    <p>{{ pricen.name }}:</p><h4>{{ pricen.price }}</h4>
                  </div>
                    ...
                    <button v-if="isAdmin"  @click="deletePrice(index)" class="delete-button">Удалить</button>
                </div>
          ...
          <!-- Форма добавления новой цены для всех пользователей -->
                ...
                <button v-if="isAdmin"  @click="savePrices" class="save-button">Сохранить изменения</button>
            </div>
        </div>
    </div>
  </div>
...
export default defineComponent({
  setup() {
    ...
    // const loadPrices = async () => {
    //   await store.dispatch('price/fetchNamePrices'); // Загружаем данные через Vuex
    //   namePrices.value = store.getters['price/namePrices']; // Получаем данные из стора
    // };
    ...
    const updatePrice = async (index: number, price: string) => {
      if (isAdmin.value) { // Проверка на права администратора
...
<style scoped>
    ...
    input.input-name, input.input-price {
      width: 100%;
      max-width: 300px;
      padding: 10px;
      margin: 10px 0;
    ...
    input.input-name:focus , input.input-price:focus  {
      border-color: #8fbc8f;
    }
