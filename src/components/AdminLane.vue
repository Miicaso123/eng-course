          ...
          <div v-if="formDataList.length > 0">
            ...
            <div v-if="testResults.length > 0" class="test-results">
                <div v-for="(result, index) in testResults" :key="index">
                  Имя: {{ result.name }}, Телефон: {{ result.phone }}, Балл: {{ result.score }} из {{ totalQuestions }}
                </div>
            </div>
...
export default defineComponent({
  setup() {
    ...
    const login = () => {
      ...
      store.dispatch('login', { email: trimmedEmail, password: password.value })
        .then(() => {
...
<style scoped>
    ...
    @media (max-width: 480px) {
        .admin-container, .admin-edit, .module-admin {
            padding: 20px;
            max-width: 100%;
        }
